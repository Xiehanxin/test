...
#define U8_GLOBAL_NVS_DATA_LENGTH               984 //Just place holder to memorize max size.
typedef struct  __attribute__((packed)) 
{
    unsigned int mu32_MagicNumber;
    unsigned short mu16_ChkSm;
    /* data in encrypted partition to be written in multiples of 16 */
    unsigned char mu8_Data[U8_GLOBAL_NVS_DATA_LENGTH]; 

} st_GlobalNvsData_t;

RTC_DATA_ATTR  st_GlobalNvsData_t gst_RetrivedNvsData;
...
RTC_NOINIT_ATTR uint32_t init_data;
RTC_NOINIT_ATTR uint32_t init_flag;
#define INIT_MAGIC 11111

/*to decide wheather the RTC parameter should be inited*/
void init_rtc_data_if_needed() {
    if (init_flag != INIT_MAGIC) {
        init_data = 1;
...
{
    esp_err_t ret;
    unsigned short lu16_Chksum;
    unsigned int Index = 0;
    unsigned char ResetCount = 20;
    // Initialize NVS.
    ...
    lu16_Chksum = calculateChecksum(gst_RetrivedNvsData.mu8_Data, U8_GLOBAL_NVS_DATA_LENGTH);
    if (gst_RetrivedNvsData.mu16_ChkSm != lu16_Chksum)
    {
        ESP_LOGE("NVRAM","%s MAGIC %X Checksum wrong %X expected %X",__func__,gst_RetrivedNvsData.mu32_MagicNumber,
            gst_RetrivedNvsData.mu16_ChkSm, lu16_Chksum);
            memset (&gst_RetrivedNvsData, 0x00, sizeof(gst_RetrivedNvsData));
        gst_RetrivedNvsData.mu32_MagicNumber = 0xDEADBEEF;
    }
    else if (gst_RetrivedNvsData.mu32_MagicNumber == 0xDEADBEEF)
    {
        memcpy (&LastValue, &gst_RetrivedNvsData.mu8_Data[U8_GLOBAL_NVS_DATA_LENGTH - 4], 4);
        ESP_LOGW("NVRAM","%s MAGIC %X Checksum Correct %X LastValue %X",__func__,gst_RetrivedNvsData.mu32_MagicNumber,
            gst_RetrivedNvsData.mu16_ChkSm, LastValue);
    }
    else
    {
        ESP_LOGW("NVRAM","%s FRESH DEVICE Start Value %X",__func__, LastValue);
    }
    
    for (Index = 0; Index < U8_GLOBAL_NVS_DATA_LENGTH; Index += 4)
    {
        LastValue ++;
        memcpy (&gst_RetrivedNvsData.mu8_Data[Index],&LastValue, 4);
    }

    gst_RetrivedNvsData.mu16_ChkSm = calculateChecksum(gst_RetrivedNvsData.mu8_Data, U8_GLOBAL_NVS_DATA_LENGTH);
    ESP_LOGW("NVRAM","%s MAGIC %X Checksum NEW %X LastValue %X",__func__,gst_RetrivedNvsData.mu32_MagicNumber,
        gst_RetrivedNvsData.mu16_ChkSm, LastValue);
    ...
    while (1)
    {
        ESP_LOGW("NVRAM", "Reset in ... %u", ResetCount);
